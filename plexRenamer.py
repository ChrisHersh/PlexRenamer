...
def renameFile(seasonCounter, seasonPath, currEpisodeName, epNum):
	newName = renamedFilePrefix + "s" + str(seasonCounter) + "e" + epNum
	if(dryRun):
		print(currEpisodeName + " --> " + newName)
	else:
		os.rename(join(seasonPath, currEpisodeName), join(seasonPath, newName))
...
def renameFiles(filetype, seasonDirs):
	...
	for s in seasonDirs:
		...
		episodes = [ f for f in listdir(seasonPath) if ( isfile(join(seasonPath,f)) and (f.endswith(videoFiletype)) ) ]
		episodes.sort()
		...
		for e in episodes:
			...
			try:
				...
				renameFile(seasonCounter, seasonPath, e, epNum)
